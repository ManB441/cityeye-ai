...
class TrafficEventBase(BaseModel):
    ...
    evidence_image: str
    details: TrafficEventDetails | RoadBlockageDetails | None = Field(default=None, exclude_if=lambda value: value is None)
    ...
    def validate_event_details_match_type(self):
        if self.event_type is EventType.ROAD_BLOCKAGE and not isinstance(self.details, RoadBlockageDetails):
            raise ValueError("ROAD_BLOCKAGE requires structured road-blockage details")
        if isinstance(self.details, RoadBlockageDetails) and self.event_type is not EventType.ROAD_BLOCKAGE:
