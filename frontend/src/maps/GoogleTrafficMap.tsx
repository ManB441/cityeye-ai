...
import { loadGoogleMaps, type GoogleMap } from "./google";

export function GoogleTrafficMap({ features, onSelect, apiKey = import.meta.env.VITE_GOOGLE_MAPS_API_KEY ?? "" }:
  { features: MapFeature[]; onSelect: (id: string) => void; apiKey?: string }) {
  ...
  useEffect(() => {
    ...
    void loadGoogleMaps(apiKey).then(sdk => {
      ...
      const instance = new sdk.Map(container.current, { center: { lat: 0, lng: 0 }, zoom: 2,
    ...
    return () => { active = false; listener?.remove(); window.removeEventListener("cityeye-map-auth-error", authFailed); };
  }, [apiKey, retry]);
  useEffect(() => {
    if (!map) return;
    map.data.forEach(feature => map.data.remove(feature));
    map.data.addGeoJson({ type: "FeatureCollection", features });
    if (!fitted.current && features.length && window.google) {
